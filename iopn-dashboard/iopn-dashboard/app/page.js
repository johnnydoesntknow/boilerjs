...
import { Star, Users, Gift, Target, ExternalLink, Heart, MessageCircle, Repeat2 } from "lucide-react"
...
export default function Dashboard() {
  ...
  const handleResetFlow = () => {
    // Clear ALL localStorage data - everything!
    ...
    // Clear tutorial data - this was missing!
    ...
    // Clear badge marketplace data including REP
    ...
    // Clear everything that starts with "iopn-" just to be sure
  ...
  useEffect(() => {
    ...
    if (isConnected && !isDiscordConnected && !discordSkipped) {
    ...
    } else if (isConnected && (isDiscordConnected || discordSkipped) && !hasNFT) {
      ...
      // Show tutorial for dashboard if everything is set up
      showTutorial("dashboard")
    }
  }, [isConnected, isDiscordConnected, hasNFT, router, showTutorial])
          ...
          <div className="min-h-screen bg-black cyber-grid hex-pattern flex items-center justify-center p-4 transition-all duration-300">
            <div className="text-center space-y-6 max-w-md w-full">
              <div className="space-y-4">
                <h1 className="text-4xl font-bold bg-gradient-to-r from-violet-indigo to-bright-aqua bg-clip-text text-transparent">
                  Welcome to IOPn
                </h1>
                <p className="text-gray-400 text-lg">Please connect your wallet to access the dashboard</p>
              </div>

              <div className="space-y-4">
                <Button
                  onClick={connectWallet}
                  className="w-full bg-gradient-to-r from-violet-indigo to-bright-aqua hover:from-violet-indigo/90 hover:to-bright-aqua/90 cyber-button pulse-glow text-white font-bold py-4 text-lg transition-all duration-300"
                  size="lg"
                >
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <header
          className={`flex h-16 shrink-0 items-center gap-2 border-b px-4 transition-all duration-300 ${
            theme === "light"
        ...
        >
          <Separator
            ...
            className={`mx-8 h-4 ${theme === "light" ? "bg-gray-300" : "bg-bright-aqua/30"}`}
          />

          <div className="flex items-center justify-between w-full">
            {/* Left - Simple welcome */}
            <div className="flex items-center space-x-4">
              <Avatar className={`h-8 w-8 border ${theme === "light" ? "border-gray-300" : "border-bright-aqua/30"}`}>
                <AvatarImage src="/placeholder.svg?height=32&width=32" />
                <AvatarFallback
                  className={`text-sm ${theme === "light" ? "bg-blue-600 text-white" : "bg-violet-indigo text-white"}`}
                >
                  U
                </AvatarFallback>
              </Avatar>
              <div>
                ...
                <p className={`text-xs ${theme === "light" ? "text-gray-700" : "text-gray-500"}`}>
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <p className={`text-sm font-bold ${theme === "light" ? "text-blue-600" : "text-bright-aqua"}`}>0 OPN</p>
              </div>
              <div className="text-right">
                <p
                  className={`text-xs uppercase tracking-wide font-medium ${theme === "light" ? "text-gray-600" : "text-gray-400"}`}
                ...
                <p
                  className={`text-sm font-bold transition-all duration-300 ${theme === "light" ? "text-gray-900" : "text-white"}`}
                >
        ...
        {/* Main Content */}
        <main
          className={`flex-1 space-y-6 p-4 md:p-6 min-h-screen transition-all duration-300 ${
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card
              className={`transition-all duration-300 ${theme === "light" ? "bg-white border-gray-200 shadow-sm" : "holo-card card-hover bg-gradient-to-br from-black/80 to-midnight-indigo/30 neon-border"}`}
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className={`text-sm font-medium ${theme === "light" ? "text-gray-600" : "text-gray-400"}`}>
                  Total REP Points
                </CardTitle>
                <Star className={`h-4 w-4 ${theme === "light" ? "text-blue-500" : "text-bright-aqua"}`} />
              </CardHeader>
              <CardContent>
                <div className={`text-2xl font-bold ${theme === "light" ? "text-blue-600" : "text-bright-aqua"}`}>
                  {userREP?.toLocaleString()}
                </div>
                <p className={`text-xs ${theme === "light" ? "text-gray-500" : "text-gray-400"}`}>+0 from yesterday</p>
              </CardContent>
            </Card>
            ...
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <Gift className={`h-4 w-4 ${theme === "light" ? "text-orange-500" : "text-amber-rust"}`} />
              </CardHeader>
              <CardContent>
                <div className={`text-2xl font-bold ${theme === "light" ? "text-orange-600" : "text-amber-rust"}`}>
                  12
                </div>
                <p className={`text-xs ${theme === "light" ? "text-gray-500" : "text-gray-400"}`}>3 new this week</p>
              </CardContent>
            </Card>
            ...
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <Users className={`h-4 w-4 ${theme === "light" ? "text-red-500" : "text-crimson-red"}`} />
              </CardHeader>
              <CardContent>
                <div className={`text-2xl font-bold ${theme === "light" ? "text-red-600" : "text-crimson-red"}`}>8</div>
                <p className={`text-xs ${theme === "light" ? "text-gray-500" : "text-gray-400"}`}>2 pending rewards</p>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent>
                      ...
                      <div className="hover:scale-105 transition-all duration-300">
                        ...
                        <div className="relative w-20 h-20 mx-auto mb-3">
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <span className="text-lg font-bold text-violet-indigo">{total}</span>
                        ...
                        <div className="text-center">
                          <h3 className="font-semibold text-white text-sm">{day.day}</h3>
              ...
              <div className="flex justify-center space-x-6 mt-6 pt-4 border-t">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-400">Gaming</span>
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`font-medium ${theme === "light" ? "text-blue-600" : "text-bright-aqua"}`}>
                          X
                        </span>
                        <Badge className={task.completed ? "bg-green-500 text-white" : "bg-amber-rust text-white"}>
                    ...
                    <p className={`text-sm font-medium mb-3 ${theme === "light" ? "text-gray-800" : "text-white"}`}>
                      {task.content}
                    </p>
                    <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            >
              ...
              <CardContent className="space-y-4">
                <Link href="/nft-mint">
                  <Button className="cyber-button pulse-glow w-full justify-start bg-gradient-to-r from-violet-indigo to-bright-aqua hover:from-violet-indigo/90 hover:to-bright-aqua/90 text-white transition-all duration-300">
          ...
          {/* REP Benefits and Top Contributors - Bottom Section STACKED VERTICALLY */}
                ...
                <div className="space-y-3">
                  {[
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${getRankColorClasses(contributor.rank, contributor.color)}`}
                        >
                          {contributor.rank}
                        </div>
                        <div className="flex flex-col">
                          <p className={`font-medium text-base ${theme === "light" ? "text-gray-800" : "text-white"}`}>
                            {contributor.name}
                          </p>
                          {contributor.isYou && (
                            <Badge
                      ...
                      <div className="text-right">
                        <p className={`text-lg font-bold ${theme === "light" ? "text-gray-800" : "text-white"}`}>
