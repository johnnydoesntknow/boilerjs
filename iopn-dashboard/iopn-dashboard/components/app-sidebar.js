    ...
    >
      <div className="h-full w-full bg-background/95 backdrop-blur-sm md:bg-transparent md:backdrop-blur-none flex flex-col">
        <SidebarHeader>
          ...
          {/* REP Balance - Only visible when wallet is connected AND Discord is connected */}
          {isConnected && isDiscordConnected && (
            <div className="px-2 pb-2 group-data-[collapsible=icon]:hidden">
              <div
                className={`${theme === "light" ? "light-mode-card" : "bg-gradient-to-r from-bright-aqua/10 to-violet-indigo/10 border border-bright-aqua/20"} rounded-lg p-3 text-center transition-all duration-300`}
              >
                <p className={`text-xs ${theme === "light" ? "text-gray-600" : "text-gray-400"} mb-1`}>REP Balance</p>
                <p className={`text-lg font-bold ${theme === "light" ? "text-bright-aqua" : "text-bright-aqua"}`}>
                  {currentRep.toLocaleString()} REP
                </p>
              </div>
            </div>
          )}

          {/* Connected Wallet Display - Only visible when wallet is connected */}
          {isConnected && (
            <div className="px-2 pb-4 group-data-[collapsible=icon]:hidden">
              <div
            ...
            <SidebarGroup>
              ...
              <SidebarGroupContent>
                <div className="px-2 space-y-3 group-data-[collapsible=icon]:hidden">
                  ...
                  <div
                    className={`${theme === "light" ? "light-mode-badge-amber-rust" : "bg-gradient-to-r from-amber-rust/10 to-amber-rust/5 border border-amber-rust/20"} rounded-lg p-3 transition-all duration-300`}
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-sm ${theme === "light" ? "text-gray-600" : "text-gray-400"}`}>Badges</span>
                      <Badge
                        variant="outline"
                        className={`${theme === "light" ? "light-mode-badge-amber-rust" : "bg-amber-rust/10 text-amber-rust border-amber-rust"}`}
                      >
        ...
        <div className="mt-auto">
          <SidebarFooter>
            {/* Disconnect Wallet Button - Only visible when wallet is connected */}
            {isConnected && (
              <div className="px-2 pb-2 group-data-[collapsible=icon]:hidden">
                <Button
            ...
            {/* Discord Disconnect Button - Only visible when Discord is connected */}
            {isDiscordConnected && (
              <div className="px-2 pb-2 group-data-[collapsible=icon]:hidden">
                <Button
